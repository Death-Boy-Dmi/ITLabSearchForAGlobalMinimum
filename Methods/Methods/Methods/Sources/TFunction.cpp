...
void TFunction::DeleteSpace(std::string Line)
{
...
std::string TFunction::CutInfix(std::string Line)
{
	std::string CutIn = "";
	std::string temp = "";
	bool Check = false;
...
int TFunction::Priority(char Operation)
{
	if (Operation == '(')
		return 0;
	else if (Operation == ')')
...
std::string TFunction::InfiToPost(void)
{
...
bool TFunction::CheckBrackets(std::string &Line)
{
...
double TFunction::CalcBin(double Fir, double Sec, char Op)
{
	if (Op == '-')
		return Fir - Sec;
	else if (Op == '+')
...
double TFunction::StandartFunc(double Fir,char Op)
{
	if (Op == 's')
		return sin(Fir);
	else if (Op == 'a')
...
TFunction::TFunction(std::string Line)
{
	SetFunction(Line);
}
...
void TFunction::SetFunction(std::string Line)
{
...
std::string TFunction::GetFunction(void)
{
	return Infix;
}
double TFunction::Calculate(const double& x)
{
	if (Infix == "")
		throw Error_1004;
	else
